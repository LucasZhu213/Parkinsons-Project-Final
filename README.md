...
Run ```logreg.py``` to train the logistic regression model based on the MDS-UPDRS data (from the 3 ```.csv``` files). Uncomment the section that merges, preprocesses and one-hot encodes the dataset. It should take a few minutes to run; the final testing and training accuracy and confusion matrices will be printed, and the final model will be saved in ```lr_model.joblib```. Before training ```gcn_model3.py```, you will need to run ```logreg.py```, and you may need to uncomment the section that trains the initial GCN+GraphNorm architecture. 
